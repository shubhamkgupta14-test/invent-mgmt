...
def build_user_response(user: dict):
    return {
        ...
        "email_verification_pending_until": format_datetime_iso(user.get("email_verification_pending_until")) if user.get("email_verification_pending_until") else None,
...
def build_invoice_response(invoice: dict):
    return {
        ...
        "invoice_status": invoice.get("invoice_status", "ACTIVE"),
        "cancel_reason": invoice.get("cancel_reason"),
        "cancelled_by": invoice.get("cancelled_by"),
        "cancelled_at": format_datetime_iso(invoice.get("cancelled_at")) if invoice.get("cancelled_at") else None,
        "email_sent_at": format_datetime_iso(invoice.get("email_sent_at")) if invoice.get("email_sent_at") else None,
        "email_sent_count": invoice.get("email_sent_count", 0),
